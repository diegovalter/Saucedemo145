...
async function fill_checkout_step(page){
    await page.locator('[data-test="checkout"]').click()
    await expect(page).toHaveURL(/checkout-step-one\.html/)
    await expect(page.locator('[data-test="title"]')).toHaveText('Checkout: Your Information')
    await page.locator('[data-test="firstName"]').fill('NomeTeste')
    await page.locator('[data-test="lastName"]').fill('SobrenomeTeste')
    await page.locator('[data-test="postalCode"]').fill('01023-003')

}

async function checkout_confirm(page){
...
async function finish_step(page){
...
test.describe('SauceDemo - fluxo principal de compra', () => {
    test('Comprar Mochila Direto', 
        ...
        // Inicio do Passo 5
        await test.step('Ir para o Checktout', async () => {
            await fill_checkout_step(page)
            await snap(page, testInfo, 'TC001-Passo05-Checkout-Preenchido')
        }) // fim do passo 5

        // Inicio do Passo 6
        await test.step('Ir para Finalizacao do Checkout', async () => {
            await checkout_confirm(page)
            await snap(page, testInfo, 'TC001-Passo06-Finalizao-Checkout')    
        }) // fim do passo 6

                // Inicio do Passo 7
        await test.step('Ir para Confirmacao da Compra', async () => {
            await finish_step(page)
            await snap(page, testInfo, 'TC001-Passo07-Confirmacao-Compra')    
        }) // fim do passo 7
